...
function Role() {
    ...
    return (
        <section className="w-full h-screen flex items-center justify-center lg:justify-between bg-(--bg-color)">
            <div className="md:w-[65%] lg:w-[50%] h-full flex-col flex items-center justify-center px-0 lg:px-14 xl:px-20">
                ...
                <form onSubmit={handleSubmit} className="w-full mb-4">
                    <div className="flex items-center gap-8 mb-8">
                        <label htmlFor="finder" className={`${active.includes('finder')? 'bg-(--secondary-color)/25 border-(--primary-color)': 'bg-(--secondary-color)' } w-[50%] flex flex-col items-center gap-3 px-8 py-14 bg-(--secondary-color) hover:bg-[#38405480] rounded-4xl transition duration-300 ease-in-out border-2 border-(--secondary-color) hover:border-(--primary-color) cursor-pointer`}>
                            <div className="w-18 h-18 flex items-center justify-center rounded-full bg-[#51c20640]">
                                <PiUserLight className="text-4xl text-(--primary-text)"/>
                            </div>
                            <h3 className="font-Plus-Jakarta-Sans text-lg capitalize text-center text-(--primary-text)">properties <br /> finder</h3>
                        </label>
                        ...
                        <label htmlFor="agent" className={`${active.includes('agent')? 'bg-(--secondary-color)/25 border-(--primary-color)': 'bg-(--secondary-color)' } w-[50%] flex flex-col items-center gap-3 px-8 py-14 bg-(--secondary-color) hover:bg-[#38405480] rounded-4xl transition duration-300 ease-in-out border-2 border-(--secondary-color) hover:border-(--primary-color) cursor-pointer`}>
                            <div className="w-18 h-18 flex items-center justify-center rounded-full bg-[#51c20640]">
                                <PiUserLight className="text-4xl text-(--primary-text)"/>
                            </div>
                            <h3 className="font-Plus-Jakarta-Sans text-lg capitalize text-center text-(--primary-text)">properties <br /> agent</h3>
                        </label>
