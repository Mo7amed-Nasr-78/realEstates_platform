...
function UserProfile() {
	...
	const listData = useMemo(() => {
		...
		if (user.role !== 'agent') {
			if (windowWidth < 992) {
				return {
					items: ['profile', 'listings', 'agents', 'sign out'],
					icons: [<PiUser />, <PiBuildings />, <PiOfficeChair/>, <PiSignOut />]
				};
			} else {
				return {
		...
		if (windowWidth < 992) {
			return {
				items: ['profile', 'listings', 'agents', 'stats', 'bookings',  'add property', 'sign out'],
				icons: [<PiUser />, <PiBuildings />, <PiOfficeChair/>, <PiChartPieSlice />, <PiTicket />, <PiUploadSimple />, <PiSignOut />]
			}
		} else {
			return {
			...
			{!user? (
				...
				<>
					<div className="flex items-center gap-2">
						...
						<div onClick={readNotifications} className="relative">
							...
							<div ref={notificationListRef} className={`${openNotifications? 'block': 'hidden'} absolute -right-10 sm:right-0 w-86 dropdown p-3 mt-2 bg-(--bg-color) outline outline-offset-1 outline-white/10 rounded-xl shadow shadow-white/10 z-10 origin-top`}>
